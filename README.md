...
Pull the image from the Docker repository.
    ...
    docker pull divisadero/airflow

## Build

Optionally install [Extra Airflow Packages](https://airflow.incubator.apache.org/installation.html#extra-package) and/or python dependencies at build time :

    docker build --rm --build-arg AIRFLOW_DEPS="datadog,dask" -t divisadero/airflow .
    docker build --rm --build-arg PYTHON_DEPS="flask_oauthlib>=0.9" -t divisadero/airflow .

or combined

    docker build --rm --build-arg AIRFLOW_DEPS="datadog,dask" --build-arg PYTHON_DEPS="flask_oauthlib>=0.9" -t divisadero/airflow .

Don't forget to update the airflow images in the docker-compose files to divisadero/airflow:latest.

## Usage

By default, airflow runs Airflow with **SequentialExecutor** :

    docker run -d -p 8080:8080 divisadero/airflow webserver
...
`LOAD_EX=n`
    ...
    docker run -d -p 8080:8080 -e LOAD_EX=y divisadero/airflow
...
For encrypted connection passwords (in Local or Celery Executor), you must have the same fernet_key. By default airflow generates the fernet_key at startup, you have to set an environment variable in the docker-compose (ie: docker-compose-LocalExecutor.yml) file to set the same key accross containers. To generate a fernet_key :
    ...
    docker run divisadero/airflow python -c "from cryptography.fernet import Fernet; FERNET_KEY = Fernet.generate_key().decode(); print(FERNET_KEY)"
...
If you want to run other airflow sub-commands, such as `list_dags` or `clear` you can do so like this:
    ...
    docker run --rm -ti divisadero/airflow airflow list_dags
...
You can also use this to run a bash shell or any other command in the same environment that airflow would be run in:
    ...
    docker run --rm -ti divisadero/airflow bash
    docker run --rm -ti divisadero/airflow ipython
